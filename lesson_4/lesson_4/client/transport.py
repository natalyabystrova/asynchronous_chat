...
from constantsnutils.utils import *
from constantsnutils.variables import *
from constantsnutils.errors import ServerError
...
class ClientTransport(threading.Thread, QObject):
    ...
    def __init__(self, port, ip_address, database, username):
        print(database, ip_address, username, port)
    ...
    def connection_init(self, port, ip):
        ...
        try:
            with socket_lock:
                ...
                message = make_message(self.transport)
                print(message)
                self.process_server_ans(message)
        except (OSError, json.JSONDecodeError) as exp:
            logger.critical('Потеряно соединение с сервером!', exp)
            raise ServerError('Потеряно соединение с сервером!')
    ...
    def contacts_list_update(self):
        ...
        with socket_lock:
            ...
            ans = make_message(self.transport)
    ...
    def add_contact(self, contact):
        ...
        with socket_lock:
            send_message(self.transport, req)
            self.process_server_ans(make_message(self.transport))
